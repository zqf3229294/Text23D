...
6. The worker keeps the live document open across generations, executes validated snippets directly, returns stable SVG object summaries for `get_view` by default, and exports `FCStd`, `STEP`, and `STL`. Native viewport PNG screenshots are available with `TEXT23D_FREECAD_WORKER_VIEW_BACKEND=gui`.
